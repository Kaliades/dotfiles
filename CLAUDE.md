...
- `bin/` — skrypty użytkownika (`.local/bin/`) — m.in. `theme` (przełącznik motywu dzień/noc) i `zen-sync` (sync configu Zen Browser przez iCloud, patrz Key Details)
...
- **Zen Browser — sync configu** (`bin/.local/bin/zen-sync`): domyka lukę po wbudowanym Firefox/Mozilla Sync (który ogarnia zakładki/hasła/historię/karty/listę rozszerzeń, ale NIE rzeczy specyficznych dla Zen ani lokalnych prefów). Kopiuje **wyselekcjonowane** pliki profilu (`prefs.js`, `zen-sessions.jsonlz4` = Workspaces, `zen-themes.json`+`chrome/` = Mods, `zen-keyboard-shortcuts.json`, `containers.json`, `search.json.mozlz4`, `permissions.sqlite`, `extension-*.json`, `xulstore.json`, `handlers.json`) — **nie cały profil** (137 MB cache/storage/widevine pomijane; snapshot ~400 KB). Magazyn = **iCloud** (`~/Library/Mobile Documents/com~apple~CloudDocs/zen-sync/`), świadomie **POZA gitem** (binarne pliki = churn). Model: jeden współdzielony snapshot, `push` z maszyny A nadpisuje → `pull` na B zaciąga (ostatni push wygrywa; `meta.txt` zapisuje host+datę, `status` pokazuje per-plik kto nowszy). Profil wykrywany z `installs.ini` (`Default=`). **Bezpieczeństwo**: `pull` twardo odmawia gdy Zen działa (`pgrep -x zen`) i robi backup nadpisywanych plików do `~/Library/Application Support/zen/zen-sync-backups/<stamp>/` (rotacja: ostatnie 5); `push` tylko ostrzega. Użycie: `zen-sync [push|pull|status|diff|help]`. **Nie rusza** stanu logowania Sync (`weave`, `signedInUser.json`) — to per-maszyna
